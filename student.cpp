...
void Student::readFromFile(std::vector<Student>& group, std::ifstream& file) {
	...
	while (std::getline(file, line)) {
		if (line.find("Факультет: ") != std::string::npos) {
			...
			std::map<std::string, int>* marks = nullptr;
			while (std::getline(file, line) && !line.empty()) {
				if (line.find(u8"#") != string::npos) {
					size_t pos_number = line.find('#');
					marks = nullptr;
					marks = new map<string, int>();
					student.markbook.push_back(marks);
					continue;
				}

				size_t pos = line.find(':');
				std::string subject = line.substr(0, pos);
				int mark = std::stoi(line.substr(pos + 2, 1));
				(*marks)[subject] = mark;
			}
...
void Student::writeToFile(vector<Student>& group, std::ofstream& file) {
	...
	for (const auto& student : group) {
		...
		file << "Группа: " << student.group << std::endl;
...
void encryption(string filename) {
	...
	string command = "openssl\\bin\\openssl.exe enc -aes-256-cbc -salt -in " + filename +
		" -out " + filename + ".enc -pass file:key.txt -pbkdf2";
...
void decryption(string filename) {
	...
	if (remove(filename.c_str()) != 0) {
		...
		/*888 //
				889 //void printFileContent(const string& filename) {
				890 // ifstream file(filename, ios::binary);
				891 // if (!file) {
				892 // cout << "[ERROR] - Cannot open file: " << filename << endl;
				893 // return;
				894 // }
				895 //
				896 // cout << "Contents of " << filename << ":\n";
				897 // string line;
				898 // while (getline(file, line)) {
				899 // cout << line << endl;
				900 // }
				901 // file.close();
				902 //}*/